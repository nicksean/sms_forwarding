...
4. 在 `code/web_src/app.js` 的 `pushTypeOptions()` / `updateTypeHint()` 中添加类型与提示
5. 运行 `python tools/build_web_assets.py` 重新生成 gzip Web 资源
...
| 方法 | 路径 | 处理函数 | Auth | 说明 |
|---|---|---|---|---|
| GET | `/` | `handleRoot()` | ✓ | gzip SPA shell |
| GET | `/assets/app.css` | `handleWebAsset()` | ✓ | gzip CSS 静态资源 |
| GET | `/assets/app.js` | `handleWebAsset()` | ✓ | gzip JS 静态资源 |
| GET | `/ui?panel=` | `handleUiPanel()` | ✓ | gzip 懒加载面板 HTML |
| GET | `/config.json` | `handleConfigJson()` | ✓ | Web 表单配置 JSON |
...
Web UI 源文件位于 `code/web_src/`。构建前运行 `python tools/build_web_assets.py`，
生成 `web_assets.cpp` 中的 gzip PROGMEM 字节数组。ESP32 只发送 gzip，浏览器解压。

面板 HTML 中的 `%PLACEHOLDER%` 由前端 `renderConfigTemplate()` 使用 `/config.json`
返回的数据填充:
...
| `%PUSH_CHANNELS%` | 前端按 `/config.json.pushChannels` 生成 5 个通道的 HTML 表单 |
...
| `/assets/app.css` `/assets/app.js` `/ui` | gzip `text/*` | 预压缩 Web 资源 |
...
- **修改页面样式**: 编辑 `code/web_src/app.css`，然后运行 `python tools/build_web_assets.py`
---

## web_src / web_assets — Web UI 源与生成资源

### 内容说明

**压缩单页应用 (SPA)**：`index.html` 是小型 shell，CSS/JS 和 9 个面板按需加载：

| 面板 ID | 名称 | 功能 |
|---|---|---|
| `panel-overview` | 系统概览 | 显示 IP/信号/配置状态 |
| `panel-sim` | 网络设置 | WiFi/蜂窝数据/SIM 设置 |
| `panel-inbox` | 收发短信 | 收件箱/已发短信/网页发短信 |
| `panel-settings` | 系统设置 | 账号、时间、维护、备份、OTA |
| `panel-push` | 推送通道 | 5 个推送通道配置 |
| `panel-keepalive` | 定时任务 | SIM 保号、定时重启、每日心跳 |
| `panel-diagnose` | 诊断与控制 | payload/USSD/模组/飞行模式 |
| `panel-atterm` | AT 终端 | AT 指令交互调试 |
| `panel-log` | 系统日志 | 实时日志查看（终端风格） |

`web_assets.cpp` 是生成文件，不手工编辑；CI 用 `tools/build_web_assets.py --check`
确认它与 `web_src` 同步。

### 修改指南

- 编辑 `code/web_src/index.html`、`app.css`、`app.js` 或 `panels/*.html`
- 需要动态内容的地方使用 `%PLACEHOLDER%`，在 `app.js::renderConfigTemplate()` 中填充
- 新增配置字段时同步 `/config.json`
- 运行 `python tools/build_web_assets.py` 重新生成 `web_assets.h/.cpp`
